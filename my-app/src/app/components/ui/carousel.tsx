...
>(({ orientation = "horizontal", opts, setApi, plugins, className, children, ...props }, ref) => {
  ...
  const handleKeyDown = React.useCallback(
    (event: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      else if (event.key === "ArrowDown") { event.preventDefault(); scrollNext() }
    },
  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <button
        ref={ref}
        className={cn(
          "absolute h-8 w-8 rounded-full flex items-center justify-center border border-white/20 bg-white/10 backdrop-blur-sm text-white hover:bg-white/20 disabled:opacity-30 transition-all",
