...
export default async function ResumePage() {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      <div className="mx-auto flex max-w-5xl flex-col gap-10 px-6 py-10 md:flex-row md:gap-14">
        {/* ── Sidebar ── */}
        ...
        <main className="flex-1 md:max-w-3xl">
          <section id="about" className="pb-10">
            <h2 className="border-b border-gray-200 pb-2 text-base font-semibold text-gray-900">About</h2>
            <div className="mt-4 flex flex-col gap-3">
              {intro.bio.split(";").map((p) => p.trim()).filter(Boolean).map((p, i) => (
            ...
            <div className="mt-4 flex flex-col gap-6">
              {research.map((paper, i) => (
                <div key={i}>
                  ...
                  {paper.abstract && (
                    <p className="mt-2 border-l-2 border-gray-200 pl-3 text-sm italic leading-6 text-gray-600">
                      {paper.abstract}
                    </p>
                  )}
